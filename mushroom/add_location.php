...
<html lang="th-TH">
<head>
	<title>MushroomFinder-เพิ่มบริเวณที่พบ</title>
...
<body>
	...
	<?php
		...
		function genkey() {
			$id = $_GET['id'];
			include 'connect.php';
        	$sql = "SELECT MushroomLocationId FROM area WHERE MushroomLocationId LIKE '%$id%' ORDER BY MushroomLocationId DESC";
    		$query = mysqli_query($conn,$sql);
    		if (mysqli_num_rows($query)<=0) {
        		return   $id."_1" ;
    		} else {
        		$result = mysqli_fetch_array($query);
        		$mushroomlocation = substr($result["MushroomLocationId"],-1);
    			$mushroomlocation = intval($mushroomlocation)+1;
        		$mushroomlocation = $id."_".$mushroomlocation;
  		...
  		<form action="insert_location.php" method="post" enctype="multipart/form-data">
			<div class="row">
				<div class="col-xl-3">
					...
					<input name="mushroom_id" type="text" value="<?=$id?>" readonly>
					<label for="mushroom_location">รหัสบริเวณ :</label>
					<input name="mushroom_location" type="text" value="<?=genkey()?>" readonly>
					<label for="txtlat">ละติจูด :</label>
					<input name="txtlat" type="text" required>
					<label for="txtlong">ลองจิจูด :</label>
					<input name="txtlong" type="text" required>
